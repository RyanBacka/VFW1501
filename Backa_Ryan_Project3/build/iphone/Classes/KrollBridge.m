...
NSString * Backa_Ryan_Project3$ModuleRequireFormat = @"(function(exports){"
...
@implementation Backa_Ryan_Project3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_backa_ryan_project3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Backa_Ryan_Project3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Backa_Ryan_Project3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_backa_ryan_project3 = [[Backa_Ryan_Project3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_backa_ryan_project3];
	
	NSString *_backa_ryan_project3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _backa_ryan_project3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_backa_ryan_project3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Backa_Ryan_Project3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.backa_ryan_project3.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.backa_ryan_project3.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
