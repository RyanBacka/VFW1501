...
//Rare exceptions to indicate a bug in the _scrollableview code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_scrollableview = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_scrollableview]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.scrollableview.module"
