...
NSString * Backa_Ryan_Project4$ModuleRequireFormat = @"(function(exports){"
...
@implementation Backa_Ryan_Project4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_backa_ryan_project4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Backa_Ryan_Project4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Backa_Ryan_Project4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_backa_ryan_project4 = [[Backa_Ryan_Project4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_backa_ryan_project4];
	
	NSString *_backa_ryan_project4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _backa_ryan_project4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_backa_ryan_project4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Backa_Ryan_Project4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.backa_ryan_project4.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.backa_ryan_project4.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
